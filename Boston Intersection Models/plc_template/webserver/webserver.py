...
@app.route('/compile_program_noauth', methods=['GET'])
def compile_program_noauth():
    global openplc_runtime
    openplc_runtime.stop_runtime()

    # find the ST file included in the directory
    fname = ''
    files = os.listdir('/workdir')
    for f in files:
        if ".st" in f.lower():
            fname = f
    os.system("cp "+os.path.join('/workdir', fname)+" ./st_files/")

    # compile with the command line
    compile_results = os.popen("cd /workdir/webserver/scripts && /workdir/webserver/scripts/compile_program.sh "+fname).read()
    all_results = {}
    all_results['compile_results'] = compile_results
    all_results['program_name'] = fname

    # set stuff in database
    prog_name = fname
    prog_descr = 'Program automatically compiled by REST API.'
    prog_file = fname
    ...
    all_results['status'] = openplc_runtime.status()

    return json.dumps(all_results)


@app.route('/start_plc_noauth')
def start_plc_noauth():
    monitor.stop_monitor()
    openplc_runtime.start_runtime()
    time.sleep(1)
    configure_runtime()
    ...
    try:
        ...
        print("This is a problem for some PLCs, but doesn't affect the running status.")
    return "{\"status\": \""+openplc_runtime.status()+"\"}"


@app.route('/stop_plc_noauth')
def stop_plc_noauth():
    openplc_runtime.stop_runtime()
    time.sleep(1)
    monitor.stop_monitor()
...
@app.route('/plc_status_noauth')
def plc_status_noauth():
    return "{\"status\": \""+openplc_runtime.status()+"\"}"
