...
class Modbus:
    ...
    def Request(self, functionId, data=''):
        ...
        print("modbus scanner contains response uid, and self uid:\n")
        print(response.unitId, self.uid)
    ...
    def DeviceInfo(self):
        ...
        # function code 0x2b is 43, see full list of function codes below
        # https://en.wikipedia.org/wiki/Modbus#Functions_and_commands
        # commented below is the original code
        # res = self.Request(0x2b, '\x0e\x01\00')
        res = self.Request(0x2b, '\x00+\x0e\x01\x00')
    ...
    if function:
        try:
            ...
            if e.code:
                ...
                return unitInfo
    else: # we only enumerate the device when the function is empty
        try:
...
def Scan(ip, port, options):
    print("modbus scanner starts with options:\n")
    print(options)
    res = False
    try:
